...
describe("Electronic Health Record Factory Test", function () {
    ...
    beforeEach(async () => {
        const ElectronicHealthRecordFactory = await ethers.getContractFactory(
            "ElectronicHealthRecordFactory"
        );
        electronicHealthRecordFactory =
            await ElectronicHealthRecordFactory.deploy();
    ...
    async function thenRecordMatchesExpected() {
        ...
        expect(
            JSON.stringify(await electronicHealthRecordFactory.getRecord(count))
        ).to.equal(JSON.stringify(output));
        expect(
            await electronicHealthRecordFactory.getDateOfBirthFromRecord(count)
        ).to.equal(record[1]);
        expect(
            await electronicHealthRecordFactory.getDateOfBirthFromRecord(
                count + 1
            )
        ).to.equal(0);
    }
