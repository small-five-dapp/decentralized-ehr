...
describe("Maintenance of validated doctors list test", function() {
    async function setup() {
        ...
        const ValidatedDoctors = await ethers.getContractFactory("ValidatedDoctorMaintenance");
    ...
    it("Cannot call addValidatedDoctor successfully if not owner.", async function() {
        ...
        await expect(contractInst.connect(addr1).addValidatedDoctor(addr1.address))
    ...
    it("Verify the correctness of the validation status of doctors.", async function() {
        ...
        const addr1DocState = await contractInst.checkValidationState(addr1.address);
        console.log("async result is: ", addr1DocState);
        await expect(contractInst.checkValidationState(addr1.address))
            .to.equal(true);
